...
3. Phase 4 `binggan` benchmarks landed (`benches/compression.rs`) with zstd-on-raw comparisons across six synthetic datasets plus six curated real-world datasets from `num_flex/test_data` (loaded via `tests/common/datasets.rs`; resolution: `OFZL_TEST_DATA_DIR` env var → `$CARGO_MANIFEST_DIR/../num_flex/test_data`; missing files are skipped with a warning). The same loader powers `tests/real_world.rs`, which round-trips every available real-world dataset including the multi-chunk `ten_value_cycle` (~16.5 MiB → 2 chunks). Initial dev-machine results: OFZL beats zstd on ratio for `ten_value_cycle` (121.7× vs 105.9×) and `synthetic_traces` (18.3× vs 16.8×, also faster encode), but trails on decode across the board and on encode for literal-heavy `wide_tail` / `all_unique`. See the bench output for the full picture.
...
- Initial emission may use explicit-offset tokens only (`offset_code = 3`). Repeated-offset token emission can be added once the baseline is correct and benchmarked, though the decoder supports it in v1.
